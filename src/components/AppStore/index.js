...
class AppStore extends Component {
  state = {searchInput: '', selectedTab: 'SOCIAL'}
  ...
  changeCategory = tabId => {
    const selected = appsList.filter(each => each.category === tabId)
    const SelectedTab = selected[0].category
  ...
  render() {
    ...
    const searchedApps = appsList.filter(
      eachApp => eachApp.category === selectedTab,
    )

    const filteredSearchedApps = searchedApps.filter(eachApp =>
          ...
          <ul>
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isSelect={eachTab.tabId === selectedTab}
              />
            ))}
          </ul>
          <div className="apps-container">
            <ul className="apps-container">
              {filteredSearchedApps.map(eachApp => (
